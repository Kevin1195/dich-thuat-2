        ...
        axios
            ...
            .then(function (response) {
                ...
                if (data.status === 'ok') {
                    ...
                    setLevelList(data.data.levelList);
                }
            })
    ...
    return (
        <div className={cx('guide')}>
            <div className={cx('guide-header')}>
                <h3 className={cx('guild-header-title')}>{t('content.guide.mucVonDauTu')}</h3>
                <div className={cx('avatar')}>
                    <img src={noavatar} className={cx('icon-avatar')} alt="" />
                </div>
                <div className={cx('header-content')}>
                    <h3 className={cx('header-content-title')}>{t('content.guide.gianHangHienTai')}</h3>
                    <div className={cx('header-content-sub-title')}>{t('content.guide.slDonHang')}</div>
                </div>
                <div className={cx('session-header')}>
                    <div className={cx('session-header-item')}>
                        <div className={cx('session-header-icon')}>
                            <svg
                                className={cx('session-header-svg')}
                                stroke="currentColor"
                                fill="currentColor"
                                strokeWidth="0"
                                viewBox="0 0 512 512"
                        ...
                        <div className={cx('session-header-desc')}>{t('content.guide.loiNhuan')}</div>
                    </div>
                    <div className={cx('session-header-item')}>
                        <div className={cx('session-header-icon')}>
                            <svg
                    ...
                    <div className={cx('session-header-item')}>
                        ...
                        <div className={cx('session-header-desc')}>{t('content.guide.cskh')}</div>
                    </div>
                </div>
                <p className={cx('guild-header-sub-title')}>{t('content.guide.thamKhao')}</p>
            </div>
            <div className={cx('guide-content')}>
                {levelList.map((item, index, elements) => {
                    const nextElement = elements[index - 1]?.name_level;
                    const currentLevel = user.roses_user.slice(-1);
                    const nextLevel = item.id_level.slice(-1);
                    if (index === 5) {
                        return;
                    } else {
                        return (
                            <div className={cx('session')} key={index}>
                                <div className={cx('session-item')}>
                                    <div className={cx('session-item-header')}>
                                        <div
                                            style={
                                                item.id_level !== user.roses_user &&
                                                currentLevel - 1 != nextLevel &&
                                                currentLevel > nextLevel
                                                    ? { flexDirection: 'column' }
                                                    : {}
                                            }
                                            className={cx('session-item-header-level')}
                                        >
                                            <div className={cx('session-item-header-name-level')}>
                                                {item.name_level}
                                            </div>
                                            {(item.id_level === user.roses_user || currentLevel < nextLevel) && (
                                                <div className={cx('session-item-header-sub-title')}>
                                                    {t('content.guide.daMo')}
                                                </div>
                                            )}

                                            {currentLevel - 1 == nextLevel && (
                                                <Link to="/system/support">
                                                    <button className={cx('session-item-button')}>
                                                        {t('content.guide.moNgay')}
                                                    </button>
                                                </Link>
                                            )}
                                            {item.id_level !== user.roses_user &&
                                                currentLevel - 1 != nextLevel &&
                                                currentLevel > nextLevel && (
                                                    <div className={cx('session-item-header-error')}>
                                                        {t('content.guide.canMoGianHang')} {nextElement}
                                                    </div>
                                                )}
                                        </div>
                                        <div className={cx('session-item-header-price')}>
                                            {formatter.format(item.price)}
                                        </div>
                                    </div>
                                    <div className={cx('session-content')}>
                                        <div className={cx('session-item-desc')}>
                                            {t('content.guide.sld')} {item.amount} {t('content.guide.donhang')}
                                        </div>
                                        <div className={cx('session-item-desc')}>
                                            {t('content.guide.loiNhuanMoiDon')} {item.roses.toFixed(1)}
                                            {t('content.guide.giaTriDonHang')}
                                        </div>
                                        <div className={cx('session-item-desc')}>{t('content.guide.slRutTien')}</div>
                                    </div>
                                </div>
                            </div>
                        );
