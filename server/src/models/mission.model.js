...
const confirmMissionID = async (token, id, id_mission, rate, review) => {
    ...
    let today = timerJoin();
    await connection.execute(
        'UPDATE mission_done SET status = 1, create_at = ?, time = ?, rate = ?, review = ? WHERE username = ? AND id = ?',
        [today, Date.now(), rate, review, username, id],
    );
